...
- **Operator surfaces**: `/admin/query` takes exactly one of `collection`/`table`; `PUT|DELETE /admin/tables/:name/rows/:id` (`?ifAbsent=1` → 409) validates STRUCTURE always but bypasses POLICY bounds (`validateRow`'s `skipPolicy` - the schema is the storage, rules are rules). Aggregates/export/import/PITR for tables arrive in S2/S3.
