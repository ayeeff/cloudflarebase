...
- Account linking trusts `google`/`github` (both attest verified emails): implicit linking on the OAuth callback stays gated on the LOCAL account being verified (the safe Better Auth default - relaxing it would let a pre-registered unverified password account capture a later social sign-in), and trust additionally allows explicit `/link-social` from a live session.
