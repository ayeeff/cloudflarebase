...
- Mail prefers the Resend-shaped HTTP transport (`RESEND_API_KEY` + `EMAIL_FROM`; `RESEND_BASE_URL` overrides the endpoint for same-shaped providers or a test sink) because it reaches ARBITRARY recipients; the `EMAIL` binding (verified destinations only) is the fallback. Sign-up/sign-in work with neither. Verification sends are best-effort (the user row already exists when the send runs); resets and invitation mail fail loud.
