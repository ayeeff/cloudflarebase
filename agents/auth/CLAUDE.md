...
- The Better Auth `organization` plugin ships for every project (anonymous users may not create orgs); invitation emails ride the same transport. New tables `organization`/`member`/`invitation` plus `session.activeOrganizationId` - compare against `getAuthTables()` on Better Auth upgrades.
