...
/**
 * Public passthrough for the db agent's join views (`POST /views/<v>/sql`).
 *
 * Same trust shape as the tables proxy - the gate lives inside the Durable
 * Object - and a view is the strictest of the three: it requires a project
 * JWT on every request and checks each member table's read permission before
 * running a statement, so `public` here means "reachable", never "open".
 * Only SELECT ever runs; there is no write surface to proxy.
 */
const proxy: RequestHandler = async ({ params, request, url, platform }) => {
	...
	const target = agentProxyUrl(url.origin, entry, projectId, '/views', params.path, url.search);
	const body =
