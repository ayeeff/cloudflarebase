		...
		<div class="flex flex-wrap items-center justify-between gap-3">
			...
			{#if status?.ok === false && status?.error}
				<span
					...
					data-testid="update-error-badge"
				>
	...
	{#if maps.length}
		<section class="mt-4 grid gap-3 sm:grid-cols-2">
			{#each maps as [slug, m] (slug)}
				<div class="rounded-lg border border-border/40 bg-background p-4 shadow-sm">
					<div class="flex items-center gap-2 text-sm font-medium">
	...
	<section class="mt-6">
		...
		>
			...
			{:else if form?.success}
				<p class="mt-2 text-xs text-emerald-600" data-testid="update-run-ok">
