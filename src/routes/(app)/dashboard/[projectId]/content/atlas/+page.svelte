<script lang="ts">
	...
	function toggleSort(key: SortKey) {
		if (sortKey === key) {
			...
			sortKey = key;
			sortDir = key === 'slug' || key === 'title' || key === 'type' ? 'asc' : 'desc';
		}
		page = 1;
	}
...
<svelte:head>
	<title>Atlases · Geo Admin · Cloudflarebase</title>
...
<div class="mx-auto max-w-full space-y-6 px-3 py-5 sm:px-6 sm:py-8">
	<div class="flex items-center justify-between">
		<div>
			<h1 class="text-2xl font-bold tracking-tight">Atlases</h1>
			<p class="text-sm text-muted-foreground">
				{data.count} atlas pages (/atlas/) · {data.deniedCount} hidden
			</p>
		</div>
		{#if totalPages > 1}
		...
		<Table.Root class="w-full table-fixed">
			<Table.Header>
				<Table.Row>
					...
					<Table.Head class="w-[22%] cursor-pointer select-none" onclick={() => toggleSort('slug')}>Slug{caret('slug')}</Table.Head>
					<Table.Head class="w-[26%] cursor-pointer select-none" onclick={() => toggleSort('title')}>Title{caret('title')}</Table.Head>
	...
	{#if data.deniedCount > 0}
		<div class="rounded-lg border border-dashed">
			<div class="border-b px-4 py-3">
				<h2 class="text-sm font-semibold">Hidden atlases (denylist)</h2>
	...
	<AlertDialog.Content>
		<AlertDialog.Header>
			...
			<AlertDialog.Description>
				The atlas page will return 404 on the live site until restored. This is reversible
				from the hidden list below.
			</AlertDialog.Description>
		</AlertDialog.Header>
		<AlertDialog.Footer>
...
<AlertDialog.Root open={!!deleteTarget} onOpenChange={(o) => !o && (deleteTarget = null)}>
	<AlertDialog.Content>
		<AlertDialog.Header>
			...
			<AlertDialog.Description>
				This removes the atlas page (src/pages/atlas/&lt;slug&gt;.astro), its data JSON and
				screenshots, and rebuilds the map index. It requires filesystem access and only
				works from a local dev checkout — if it fails here, use
				<span class="font-medium">Hide</span> instead.
			</AlertDialog.Description>
		</AlertDialog.Header>
		<AlertDialog.Footer>
			<AlertDialog.Cancel>Cancel</AlertDialog.Cancel>
...
<AlertDialog.Root open={!!restoreTarget} onOpenChange={(o) => !o && (restoreTarget = null)}>
	<AlertDialog.Content>
		<AlertDialog.Header>
			...
			<AlertDialog.Description>
				The atlas page will be removed from the denylist and live again on the site.
			</AlertDialog.Description>
		</AlertDialog.Header>
		<AlertDialog.Footer>
			<AlertDialog.Cancel>Cancel</AlertDialog.Cancel>
